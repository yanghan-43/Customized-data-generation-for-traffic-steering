...
class PersonalEquipment(object):
    ...
    def assign_handover_params(self, handover_setting, radius):
        self.handover_param = handover_setting[self.pe_id]  # the handover setting for one pe
        self.update_handover_params(radius)

    def update_handover_params(self, radius):
        self.handover_mean = self.load * (BASE_MEAN * radius) * self.handover_param[self.handover_mode]  #the handover mode under one mode
        self.handover_var = self.load * (BASE_VAR * radius) * self.handover_param[self.handover_mode]
